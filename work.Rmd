...
evaluate_garch <- function(ticker, data, mean.model, variance.model) {
  ...
  tibble(ticker = ticker, fit = c(garch_fit), AIC = infocriteria(garch_fit))
}
...
make_asset_garchs <- function(asset_returns, garchOrders, tickers_filter_table) {
  if (nrow(garchOrders) > 0 & nrow(asset_returns) > 0) {
    ...
    for (t in tickers) {
      ...
      models <- models %>% 
        add_row(evaluate_garch(t, ret, mean.model, variance.model))
    }
...
make_asset_garchs(asset_returns, garchOrders, tickers_filter_table)
```
